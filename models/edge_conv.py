...
class DynamicEdgeConv_DGM(EdgeConv):
    ...
    def sample_edges(self, nodes_emb):
        ...
        # Maybe cosinus can work better
        # sim = 1 + torch.cosine_similarity(nodes_emb, nodes_emb.unsqueeze(1), dim=-1)
        # probs = torch.exp(-self.temperature * sim) + self.eps
...
class DynamicEdgeConvMINE(EdgeConvMINE):
    def __init__(self, in_channels, out_channels, k):
        super().__init__(in_channels, out_channels, k)
        self.k_degree = k
        self.mapper = Seq(Linear(in_channels, in_channels),
