...
class analize_gcnEG():
    ...
    def temperature_stat(self, model):
        ...
        for idx in range(len(model)):
            if isinstance(model[idx], GCNConv):
                self.d[f"temperatue_{idx}"] =  model[idx].temperature.clone().detach()[0]
            else:
                pass
    ...
    def log_A_probs_masks(self, model, batch):
        ...
        for idx in range(len(model)):
            if isinstance(model[idx], GCNConv):
                probs = model[idx].probs.clone().detach().cpu()
                mask = model[idx].mask.clone().detach().cpu()
